...
static int32_t loopback_packet_check(const char *src_buf) {
  ...
  if (adapter_info == NULL) {
    coap_log(LOG_ERR, "adapter_info malloc failed\n");
    return 1;
  }
  ...
  if (err != NO_ERROR) {
    coap_log(LOG_ERR, "GetAdaptersInfo failed with error: %d\n", err);
    goto L_IS_LOOPBACK;
  }
...
coap_endpoint_get_session(coap_endpoint_t *endpoint,
  ...
  if (is_loopback_packet(packet) == 1) {
    coap_log(LOG_DEBUG, "coap_endpoint_get_session: drop loopback packet");
    return NULL;
  }
#endif
